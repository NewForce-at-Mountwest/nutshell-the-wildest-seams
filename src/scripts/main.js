...
document.querySelector("body").addEventListener("click", function () {
  ...
  // Fetch for one task
  const getOneTask = (id) => {
    fetch(`http://localhost:3000/tasks/${id}`).then((r) => r.json());
  };
  ...
  if (event.target.id === "new-task-btn") {
    ...
    fetch("http://localhost:3000/tasks?completed=false")
      ...
      .then((tasks) => {
        // Print New Task Card
        taskAreaSelector.innerHTML += newCardHTML();
    ...
    // Form values
    // Task name value
    const newTaskNameValue = document.querySelector("#new-task-text-form")
      .value;
    // Task date value
    const newTaskDateValue = document.querySelector("#new-task-date-form")
      .value;
    // JSON object
    const taskObject = {
    ...
    document
      .querySelector(`#card-${taskPrimaryKey}`)
    ...
    // Edit form date value
    const editDateValue = document.querySelector("#edit-task-date-form").value;
